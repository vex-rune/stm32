...
{
    printf("ESP32 Module Initializing...\r\n");
    ...
    esp32_read_resp(rx_buf, &rx_len, sizeof(rx_buf), 1000);
    ...
    ESP_StatusTypeDef reset = esp32_reset();

    printf("ESP32 Module Initialized.\r\n");
...
ESP_StatusTypeDef esp32_read_resp(uint8_t buf[], uint16_t* len, uint16_t size, uint32_t timeout_ms)
{
    ...
    while ((HAL_GetTick() - tick_start) < timeout_ms)
    {
        ...
        // 空闲帧检测：至少收到2字节才算有效响应，避免噪声误触发
        if (rx_len >= 2 && (HAL_GetTick() - last_rx_tick) > 100)
        {
            break;
        }
    }

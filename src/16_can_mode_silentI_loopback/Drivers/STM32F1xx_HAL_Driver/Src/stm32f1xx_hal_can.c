...
/**
  ...
  * @param  hcan 指向 CAN_HandleTypeDef 结构体的指针，包含指定 CAN 的配置信息
  * @retval HAL 状态
  *
  * @note 配置说明:
  *       - Prescaler: 预分频器，决定 CAN 时钟分频系数
  *       - Mode: CAN 工作模式（正常/静默/回环/静默回环）
  *       - SyncJumpWidth: 同步跳转宽度，用于重同步
  *       - TimeSeg1 (BS1): 相位缓冲段1，影响采样点位置
  *       - TimeSeg2 (BS2): 相位缓冲段2，影响采样点位置
  *       - TimeTriggeredMode: 时间触发通信模式（禁用）
  *       - AutoBusOff: 自动总线关闭管理
  *       - AutoWakeUp: 自动唤醒模式
  *       - AutoRetransmission: 自动重传模式
  *       - ReceiveFifoLocked: 接收 FIFO 锁定模式
  *       - TransmitFifoPriority: 发送 FIFO 优先级模式
  */
HAL_StatusTypeDef HAL_CAN_Init(CAN_HandleTypeDef *hcan)
{
  ...
  /* 参数合法性检查 - 确保各配置值在有效范围内 */
  assert_param(IS_CAN_ALL_INSTANCE(hcan->Instance));                /* CAN 外设实例检查 */
  assert_param(IS_FUNCTIONAL_STATE(hcan->Init.TimeTriggeredMode));   /* 时间触发模式参数 */
  assert_param(IS_FUNCTIONAL_STATE(hcan->Init.AutoBusOff));          /* 自动总线关闭参数 */
  assert_param(IS_FUNCTIONAL_STATE(hcan->Init.AutoWakeUp));          /* 自动唤醒参数 */
  assert_param(IS_FUNCTIONAL_STATE(hcan->Init.AutoRetransmission));  /* 自动重传参数 */
  assert_param(IS_FUNCTIONAL_STATE(hcan->Init.ReceiveFifoLocked));   /* 接收FIFO锁定参数 */
  assert_param(IS_FUNCTIONAL_STATE(hcan->Init.TransmitFifoPriority)); /* 发送FIFO优先级参数 */
  assert_param(IS_CAN_MODE(hcan->Init.Mode));                        /* CAN 模式参数 */
  assert_param(IS_CAN_SJW(hcan->Init.SyncJumpWidth));                /* 同步跳转宽度参数 */
  assert_param(IS_CAN_BS1(hcan->Init.TimeSeg1));                     /* BS1时间段参数 */
  assert_param(IS_CAN_BS2(hcan->Init.TimeSeg2));                     /* BS2时间段参数 */
  assert_param(IS_CAN_PRESCALER(hcan->Init.Prescaler));             /* 预分频器参数 */
  ...
  /* 设置自动重传模式 (NART) - 发送失败自动重试 */
  /* 注意: NART 位为 1 时禁用自动重传，为 0 时启用 */
  ...
  /* 设置接收 FIFO 锁定模式 (RFLM) - FIFO 满时不覆盖旧数据 */
  ...
  /* 设置发送 FIFO 优先级 (TXFP) - 按优先级而非邮箱号发送 */
  ...
  /* 配置位时序寄存器 (BTR) - 设置波特率和采样点 */
  /* 包含: 模式、SJW、BS1、BS2、预分频器 */

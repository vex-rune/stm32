...
/**
  * @brief  进入睡眠模式
  * @note   在睡眠模式下，所有I/O引脚保持运行模式下的电平状态不变。
  * @param  Regulator：调压器状态，该参数在睡眠模式下无实际作用，仅用于兼容旧版软件，保证代码可移植性
  * @param  SLEEPEntry：指定使用 WFI 还是 WFE 指令进入睡眠模式。
  *         如果使用 WFI 方式进入睡眠，若不希望 SysTick 定时器中断作为唤醒源，则需要关闭该定时器中断。
  *         该参数可选以下值：

...
UART 引脚：TX/RX PA9/PA10。

SPI 引脚：SCK/MISO/MOSI/CS  PA5/PA6/PA7/PA4。
...
LibDriver 同时提供 Keil 和 IAR 集成开发环境工程。

MDK 是 Keil ARM 工程，您的 Keil 版本必须为 5 或更高。Keil ARM 工程需要 STMicroelectronics STM32F4 系列器件包，可从 https://www.keil.com/dd2/stmicroelectronics/stm32f407zgtx 下载。

EW 是 IAR ARM 工程，您的 IAR 版本必须为 9 或更高。
...
我们使用 `\n` 进行换行。如果您的串口助手显示异常（例如显示的内容不换行），请修改串口助手的配置，或更换一个支持 `\n` 解析的串口助手。
